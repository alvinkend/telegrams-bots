...
bot.onText(/\/start|\hi/, msg => {
  bot.sendMessage(msg.chat.id, "Ada yang bisa dibantu");
  bot.sendMessage(msg.chat.id, "Silahkan isi data anda:");
  bot.sendMessage(msg.chat.id, `*No Telepon*-*Nama*-*Alamat*`, {
    parse_mode: "Markdown"
  });

  bot.sendMessage(msg.chat.id, `*08123123123*-*aji*-*Jakarta*`);
});
...
bot.onText(/\d/, msg => {
  ...
  const addUser = () =>
      ...
      .post(API_URL + Customer, {
        ...
        bot.sendMessage(
					msg.chat.id,
					`Selamat *${name}*, data anda telah tersimpan. Silahkan lannjutkan pesanan.`,
					{ parse_mode: "Markdown" }
				);
      })
			.catch(err => {
	...
	axios.get(API_URL + Customer + id)
		.then(response => {
			if (!response.data.data) {
				...
				bot.sendMessage(
					msg.chat.id,
					`*${name}*, data anda sudah tersimpan. Silahkan lanjutkan pesanan.`,
					{ parse_mode: "Markdown" }
				);
			}
		})
